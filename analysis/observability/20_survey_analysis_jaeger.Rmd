...
```{r validate-params}
# If running in RStudio (which defaults working dir to the location of the Rmd file), create a directory somewhere (e.g. in the root of the repo), and use it at param, e.g. '../../output')
...
No problems with the estimated Pareto-k values, they are well below 0.7.
...
No problems with the LOO-CV values, all Pareto-k values are below 0.7, though some are between 0.5--0.6.
...
All Pareto-k values are good, aroun 0.2 or lower. Well below the limit 0.7.
...
All Pareto-k values are good, highest are around 0.6. and some other around 0.5.
But none close to the limit 0.7.
...
All Pareto-k values are OK, around 0.3 or lower, well below 0.7.
...
One observation above the limit of 0.7.
...
Running reloo, which does exact LOO-CV (leaving out one problematic observation at a time) reveals that all Pareto-k estimates are below 0.7, so we should be able to use the model inferences.
...
Pareto-k values are good also, max value around 0.4.
...
Example of running with narrower standard deviation prior (exp(10) instead of our default exp(1))
...
LOO-CV says that all values are good, some Pareto-k are around 0.6, but all are less than 0.7.
